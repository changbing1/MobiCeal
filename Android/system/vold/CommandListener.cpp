...
int CommandListener::CryptfsCmd::runCommand(SocketClient *cli,
    ...
    /* Bing : Added cli option to enable pde */
    } else if (!strcmp(argv[1], "pde")) {
        if ( (argc != 5 && argc != 6) || (strcmp(argv[2], "wipe") && strcmp(argv[2], "inplace")) ) {
            cli->sendMsg(ResponseCode::CommandSyntaxError, "Usage: cryptfs pde <wipe|inplace> <passwd_outer> <passwd_hidden> <passwd_del>", false);
            return 0;
        }
        SLOGD("cryptfs enablecrypto %s {}", argv[2]);
	if (argc == 5)
        	rc = cryptfs_enable_pde(argv[2], argv[3], argv[4], NULL);
	else if (argc == 6)
