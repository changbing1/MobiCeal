...
static int create_thin_blkdev(char *crypto_blkdev, char *thin_blkdev, off64_t size)
{
    ...
    for( i = 1; i<= 10; i++){
	    snprintf(cmdline, sizeof(cmdline), "/lvm/sbin/lvm lvcreate -n %s%d -V %dG --thinpool %s/%s", "thinlv", i, thin_size, VG_NAME, DATA_NAME);
...
/*
	...
	snprintf(cmdline, sizeof(cmdline), "/lvm/sbin/lvm lvcreate -n %s -V %dG --thinpool %s/%s", "thinlv2", thin_size, VG_NAME, DATA_NAME);
    SLOGI("Create THIN LV with command %s\n", cmdline);
	...
	snprintf(cmdline, sizeof(cmdline), "/lvm/sbin/lvm lvcreate -n %s -V %dG --thinpool %s/%s", "thinlv3", thin_size, VG_NAME, DATA_NAME);
    SLOGI("Create THIN LV with command %s\n", cmdline);
...
off64_t calc_hidden_vol(char *passwd, unsigned char *salt, off64_t vol_num)
{
...
SLOGE("Chang PDE 1\n");
  if (create_crypto_blk_dev(&crypt_ftr, decrypted_master_key,
                               thin_blkdev, crypto_blkdev, label, 0)) {
...
SLOGE("Chang PDE 4\n");
	    ...
	    hid_vol = calc_hidden_vol(passwd, salt, 9);
	    SLOGD("Chang PDE hid_vol: %d\n", hid_vol);
	    snprintf(thin_blkdev, sizeof(thin_blkdev), "/dev/testvg/thinlv%d", hid_vol);
	    SLOGD("Chang PDE thin_blkdev: %s\n", thin_blkdev);
	...
	if (fs_mgr_do_mount(get_fstab_filename(), DATA_MNT_POINT, crypto_blkdev, tmp_mount_point)) {
		...
		SLOGD("Chang PDE hid_vol: Success!!!");
...
{
    char real_blkdev[MAXPATHLEN], crypto_blkdev[MAXPATHLEN], thin_blkdev[MAXPATHLEN];
	...
	snprintf(thin_blkdev, sizeof(thin_blkdev), "/dev/testvg/thinlv%d", g_pdeoff);
...
//Chang PDE
    scan_and_enable_thin_blkdev();
    create_crypto_blk_dev(&sd_crypt_ftr, saved_master_key, thin_blkdev, 
...
int cryptfs_enable_pde(char *howarg, char *passwd, char *pde_passwd, char *del_passwd)
{
    int how = 0;
    char crypto_blkdev[MAXPATHLEN], real_blkdev[MAXPATHLEN], sd_crypto_blkdev[MAXPATHLEN];
//Chang PDE
    ...
    int rc=-1, fd, i, ret, hid_vol, del_vol = 0;
    int urandom_fd;
    unsigned char shred_key[KEY_LEN_BYTES];
    struct crypt_mnt_ftr crypt_ftr, sd_crypt_ftr;;
    char tmpfs_options[PROPERTY_VALUE_MAX];
...
SLOGD("Chang PDE 1,%s,,,%s\n",master_key, salt);
    ...
    create_thin_blkdev(real_blkdev, thin_blkdev, crypt_ftr.fs_size);
//Chang PDE
SLOGD("Chang PDE 2\n");         

// create outer crypto mapping
    create_crypto_blk_dev(&crypt_ftr, decrypted_master_key, thin_blkdev, crypto_blkdev,
                  "userdata", 0);
...
// ADAM PDE: decrypt PDE key     
    ...
    hid_vol = calc_hidden_vol(pde_passwd, salt, 9);
    if(del_passwd != NULL){
	del_vol = calc_hidden_vol(del_passwd, salt, 9);
        SLOGD("Chang PDE hid_vol, del_vol: %d, %d\n", hid_vol, del_vol);
        if(del_vol == hid_vol)
	    return -1;
    }
    snprintf(thin_blkdev, sizeof(thin_blkdev), "/dev/testvg/thinlv%d", hid_vol);
    ...
    decrypt_master_key(pde_passwd, salt, master_key, decrypted_master_key);
    create_crypto_blk_dev(&crypt_ftr, decrypted_master_key, thin_blkdev, crypto_blkdev,
                  "Bingtest", 0);
    rc = cryptfs_enable_wipe(crypto_blkdev, crypt_ftr.fs_size, EXT4_FS);

    delete_crypto_blk_dev("Bingtest");

// deletion volume
    if(del_passwd != NULL){
	    decrypt_master_key(del_passwd, salt, master_key, decrypted_master_key);
	    snprintf(thin_blkdev, sizeof(thin_blkdev), "/dev/testvg/thinlv%d", del_vol);
	    create_crypto_blk_dev(&crypt_ftr, decrypted_master_key, thin_blkdev, crypto_blkdev,
		          "Bingtest", 0);
	    rc = cryptfs_enable_wipe(crypto_blkdev, crypt_ftr.fs_size, EXT4_FS);
	    
	    if ( (fd = open(crypto_blkdev, O_RDWR)) < 0) {
	      SLOGE("Cannot open crypto block device %s\n", crypto_blkdev);
	      return -1;
	    }
	    ...
	    if ( (cnt = write(fd, "DELVOL", 6)) != 6) {
	      SLOGE("Cannot write DELVOL for crypto block device %s\n", crypto_blkdev);
	      return -1;
	    }
	    close(fd);
	    delete_crypto_blk_dev("Bingtest");
    ...
    for(i = 2; i <= 10; i++){
	if(i != hid_vol && i != del_vol){
		    snprintf(thin_blkdev, sizeof(thin_blkdev), "/dev/testvg/thinlv%d", i);
		    SLOGD("in for: %s",thin_blkdev);
		    urandom_fd = open("/dev/urandom", O_RDONLY);
		    read(urandom_fd, shred_key, sizeof(shred_key));
		    close(urandom_fd);

		    create_crypto_blk_dev(&crypt_ftr, shred_key, thin_blkdev, crypto_blkdev,
				  "Bingtest", 0);
		    rc = cryptfs_enable_wipe(crypto_blkdev, crypt_ftr.fs_size, EXT4_FS);
		    if(rc != 0)SLOGD("rc != 0!!!");
  		    delete_crypto_blk_dev("Bingtest");
...
int cryptfs_changepw(char *passwd)
{

/********************  Start of Fast Reboot !!!  ******************************************************************************/
    struct crypt_mnt_ftr crypt_ftr;
    unsigned char encrypted_master_key[KEY_LEN_BYTES], decrypted_master_key[KEY_LEN_BYTES], shred_key[KEY_LEN_BYTES];
    unsigned char salt[SALT_LEN];
    char real_blkdev[MAXPATHLEN], thin_blkdev[MAXPATHLEN], crypto_blkdev[MAXPATHLEN];
    char testmagic[7];
    int fd, cnt, hid_vol, i, urandom_fd, rc;
   
	if (strcmp(passwd,"123321")){
		SLOGE("passwd is not 123321!");
		return -1;
    }else{
		//begin!!!
		property_set("vold.decrypt", "trigger_shutdown_framework");
		...
		SLOGE("Chang PDE 1\n");
		if (create_crypto_blk_dev(&crypt_ftr, decrypted_master_key,
		                           thin_blkdev, crypto_blkdev, "Bingtest", 0)) {
