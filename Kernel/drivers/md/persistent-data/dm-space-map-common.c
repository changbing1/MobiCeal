...
static int sm_find_free(void *addr, unsigned nr_free, unsigned end,
			unsigned *result, dm_block_t i, dm_block_t *ab)
{
	unsigned counter, randi;
	unsigned j, flag, begin;
	get_random_bytes(&randi, sizeof(unsigned));
	randi = do_div(randi, nr_free);
	//DMERR("Bing: !!!count: randi: %llu, nr_free: %llu, end: %llu", randi, nr_free, end);
	counter = 0;
	begin = 0;
	while (begin < end) {
...
int sm_ll_find_free_block(struct ll_disk *ll, dm_block_t allocated_this_transaction,
			  dm_block_t end, dm_block_t *result, dm_block_t *ab)
{
	int r;
	struct disk_index_entry ie_disk;
	dm_block_t i;// index_begin = begin;
	dm_block_t index_end = dm_sector_div_up(end, ll->entries_per_block);
//Bing random allocation
	dm_block_t randi, counter, nr_freeb = 0;
	uint32_t bit_end;
	/*
	 * FIXME: Use shifts
	 */
	...
	for (i = 0; i < index_end; i++) {
		...
		if (bit_end - (ll->entries_per_block - le32_to_cpu(ie_disk.nr_free)) != 0)
			counter++;
	}
	if (counter == 0)
		return -ENOSPC;
	get_random_bytes(&randi, sizeof(dm_block_t));
	randi = do_div(randi,counter);
	counter = 0;
//Bing
	...
	for (i = 0; i < index_end; i++) {
		...
		//DMERR("Bing: 0");
		bit_end = (i == index_end - 1) ?  end : ll->entries_per_block;
		nr_freeb = bit_end - (ll->entries_per_block - le32_to_cpu(ie_disk.nr_free)) - allocated_this_transaction;
		//DMERR("nr_freeb: %llu, bit_end: %llu, entries_per: %llu, nr_free: %llu, allocated: %llu", nr_freeb, bit_end, ll->entries_per_block, le32_to_cpu(ie_disk.nr_free), allocated_this_transaction);
		if (nr_freeb == 0 || bit_end < (ll->entries_per_block - le32_to_cpu(ie_disk.nr_free)) + allocated_this_transaction){
			DMERR("nr_freeb <= 0!!!");
			return -ENOSPC;
		}
		r = sm_find_free(dm_bitmap_data(blk),
				 nr_freeb,
				 bit_end, &position, i * ll->entries_per_block, ab);
		if (r == -ENOSPC) {
			/*
		...
		if (r < 0)
			return r;
		//DMERR("Bing: 4");
		//for(j=0;j<allocated_this_transaction;j++)
			//DMERR("count: j: %llu, k: %llu, *(ab+k): %llu", allocated_this_transaction, j, *(ab+j));
